...
This repository is the layer Bakobo wrote. It sits on two projects that are not ours: [didwebvh-py](https://github.com/decentralized-identity/didwebvh-py), DIF's reference implementation of the `did:webvh` log state machine, and [keripy](https://github.com/WebOfTrust/keripy). A flaw in log verification, Data Integrity proof handling or JCS canonicalization belongs to [DIF](https://github.com/decentralized-identity/didwebvh-py/security), and a flaw in KERI event processing, CESR parsing or key-state validation belongs to [WebOfTrust](https://github.com/WebOfTrust/keripy/security); reporting it there reaches the people who can fix it.

What is in scope here is everything this repository decides. That is what the publication gate accepts and what it refuses, the conformance check that rejects log entries the `did:webvh:1.0` method version forbids even when the reference implementation is what produced them, the check that a DID claiming a KERI sibling really is under that AID's current key, and the refusal to leave a partial artifact tree behind. The `did:webvh` method itself is a [DIF specification](https://github.com/decentralized-identity/webvh_gate); report a flaw in the method there.
